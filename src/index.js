  ...
  async fetch(request, env) {
    ...
    try {
      switch (url.pathname) {
        ...
        case "/api/test":     return json(await volTest(url.searchParams.get("t"), url.searchParams.get("m"), env));
...
async function ingest(body, env) {
  ...
  // Resume: a row only counts as done if it has chunk vectors. Rows written
  // before the max-shift metric existed get re-ingested once.
  ...
  for (const kind of Object.keys(SECTIONS)) {
    ...
    const parts = chunk(clean).slice(0, 60);
...
const WINDOW = 30;
const METRICS = ["pooled", "max", "p75"];

async function volTest(which, metric, env) {
  const m = METRICS.includes(metric) ? metric : "max";
  ...
  for (const ticker of tickers) {
    ...
    const nov = (await novelty(ticker, env)).mdna.filter((r) => r[m] != null);
    const rows = [];
    ...
    for (const f of nov) {
      ...
      rows.push({ filed: f.filed, form: f.form, novelty: f[m], fwd, trail, ratio: fwd / trail });
    }
  ...
  return { metric: m, window: WINDOW, perTicker, pooled, missingPrices: missing };
}
...
<div class="sec">
  ...
  <label>Metric</label>
  <select id="metric">
    <option value="max" selected>max shift — furthest unmatched passage</option>
    <option value="p75">p75 shift — broad rewrite</option>
    <option value="pooled">pooled — section average</option>
  </select>
...
$('#run').onclick=async()=>{
  ...
  try{
    const d=await (await fetch('/api/test?t=ALL&m='+$('#metric').value)).json();
...
function renderTest(d){
  ...
  if(d.pooled){
    ...
    h+='<div class="sec"><h2>Pooled · '+p.n+' observations · '+d.metric+'</h2>'+
       ...
       '</table>'+
       '<div class="note">'+(sig
         ? 'Below 0.05. One test on a small, correlated sample — worth pursuing, not a finding. These companies move together, so the effective sample is smaller than n suggests.'
         : 'Not significant. This metric does not track forward volatility here.')+
       '<br><br>Three metrics are selectable, so if you run all three and report the best one, '+
       'you have roughly tripled your chance of a false positive. Decide which metric you '+
       'believe in before you look, or treat 0.05 as 0.017.</div></div>';
  }else{
...
function render(data,ticker){
  ...
  for(const k of Object.keys(data)){
    ...
    html+='<div class="sec"><h2>'+ticker+' · '+titles[k]+'</h2><table>'+
      '<tr><th>Filed</th><th>Form</th><th>Pooled</th><th>Max</th><th>p75</th><th style="width:26%"></th></tr>';
    for(const r of rows){
  ...
  html+='<div class="note"><b>Max</b> is the distance from each passage in this filing to its '+
    'nearest passage in the previous same-form filing, taking the largest — it answers '+
    '"is there something here with no counterpart last quarter?"<br><br>'+
    '<b>Pooled</b> averages the whole section into one vector, which drowns a single changed '+
    'paragraph in the thirty that did not change. On synthetic data with one new paragraph '+
    'injected, max moves 11.6× while pooled moves 2.6×.<br><br>'+
    '<b>p75</b> only rises when much of the section is rewritten, so max-high-but-p75-flat '+
    'means one new thing, and both high means a broad rewrite.</div>';
  $('#out').innerHTML=html;
